...
const pokeRandomImageUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${randomPokemon}.png`;

const buttons = Array.from({length: 4}, (_, i) => {
  const randomNumber = Math.floor(Math.random() * pokemons.length);

  return <button key={i}>{pokemons[randomNumber].name}</button>;
});
  ...
  return (
    <>
      <div className="container">
        <img
          src={pokeRandomImageUrl}
          alt="pokemon"
          style={{filter: "brightness(0)"}}
        />
        <div className="container-btn">{buttons}</div>
      </div>
    </>
  );
}
