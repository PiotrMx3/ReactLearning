...
const imgUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${pokemons[random].id}.png`;

const random1 = Math.floor(Math.random() * 151);
const random2 = Math.floor(Math.random() * 151);
const random3 = Math.floor(Math.random() * 151);
const random4 = Math.floor(Math.random() * 151);

function App() {
  return (
    <>
      <img style={{filter: "brightness(0)"}} src={imgUrl} alt="pokemon" />
      <button>{pokemons[random1].name}</button>
      <button>{pokemons[random2].name}</button>
      <button>{pokemons[random3].name}</button>
      <button>{pokemons[random4].name}</button>
    </>
  );
}
